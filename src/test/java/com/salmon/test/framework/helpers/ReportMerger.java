...
class ReportMerger {
	...
	private void mergeReports(File reportDirectory) throws IOException {
		...
		copyImagesToInitialMergeFolder(reportDirectory);
	}
	...
	private void getListOfFailedScenarios(File reportDirectory) throws IOException {
		...
		for (File rerunSourceReport : rerunSourceReports) {
			rerunFailedScenariosList.add(FileUtils.readFileToString(rerunSourceReport,Charset.defaultCharset()));
		}
		...
		FileUtils.writeStringToFile(mergedRerunReport, failedScenarios,Charset.defaultCharset());

	}

	private void copyImagesToInitialMergeFolder(File reportDirectory) throws IOException{
		File mergeDirectory = new File(reportDirectory.getCanonicalPath() + "/initialMerge");
		...
		for (File image : embeddedImages) {
			FileUtils.copyFileToDirectory(image, mergeDirectory);
		}
	}
	/**
